...
public class ServiceAtualizaCache
{
    ...
    public ServiceAtualizaCache(IDistributedCache cache, ILogger<ServiceAtualizaCache> logger, IConfiguration _configuration)
    {
        _cache = cache;

        _logger = logger;
        _stringConexao = _configuration.GetConnectionString("conexao-cache");
    ...
    public async Task<List<ModelWeb.Categoria>> ObterCategoria()
    {
        try
        {
            ...
            if (dadosCache != null)
            {
                ...
                return JsonSerializer.Deserialize<List<ModelWeb.Categoria>>(
                    Encoding.UTF8.GetString(dadosDescomprimidos));
            }

            return await _circuitBreaker.ExecuteAsync(async () =>
            {
                var categorias = new List<ModelWeb.Categoria>();
                ...
                {
                    categorias.Add(new ModelWeb.Categoria
                    {
